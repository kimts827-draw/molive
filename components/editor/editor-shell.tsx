...
import { ArrowLeft, Copy, Eye, EyeOff, GripVertical, History, ImagePlus, LoaderCircle, Maximize2, Minimize2, Monitor, Plus, Redo2, Save, Send, Smartphone, Sparkles, Tablet, Trash2, Undo2, X } from "lucide-react";
  ...
  return (
    <main className={`editor-app${previewFullscreen ? " preview-fullscreen" : ""}`}>
      <header className="editor-topbar">
        ...
        <div className="topbar-actions"><button className="preview-fullscreen-button" onClick={() => setPreviewFullscreen((current) => !current)} title={previewFullscreen ? "Editor 화면으로 돌아가기" : "전체화면 보기"}>{previewFullscreen ? <Minimize2 size={16} /> : <Maximize2 size={16} />} {previewFullscreen ? "Editor로 돌아가기" : "전체화면 보기"}</button><button disabled={!past.length} onClick={undo} title="실행 취소"><Undo2 size={16} /></button><button disabled={!future.length} onClick={redo} title="다시 실행"><Redo2 size={16} /></button><button onClick={() => setShowVersions(true)}><History size={15} /> 버전</button><Link className="new-design-button" href="/"><Sparkles size={15} /> 새 디자인</Link><button className="publish-button" onClick={() => setShowPublish(true)}>게시</button></div>
      </header>
