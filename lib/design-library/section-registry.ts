...
export const SECTION_TYPES: Record<SectionTypeId, SectionTypeDefinition> = {
  categoryGrid: {
    ...
    variants: {
      "quick-menu": variant("quick-menu", "category", "아이콘 퀵메뉴", ["category/quick-menu"], "원형 또는 라운드 썸네일과 라벨의 카테고리 그리드(PC 6~10개 1행, 모바일 2행 또는 가로 스크롤). 각 항목은 /product/list.html 링크. 짧은 안내 카피 한 줄을 위에 둔다."),
  ...
  featuredProducts: {
    ...
    variants: {
      "heading-more": variant("heading-more", "product-context", "타이틀+more 프레임", ["product-context/heading-more"], "상품 슬롯 위에 영문 라벨과 국문 타이틀 페어, 아래 또는 우측에 more 링크. 레퍼런스에서 가장 보편적인 진열 프레임."),
      ...
      "quiet-grid": variant("quiet-grid", "product-context", "무헤딩 정숙 진열", ["product-context/heading-more", "story/lookbook-row"], "장식적인 헤딩 없이 아주 작은 라벨 하나와 넓은 여백만으로 상품 슬롯을 연다. 앞 섹션이 이미 말을 많이 했을 때 쓰는 정숙한 진열."),
    },
  },
  productFocus: {
    id: "productFocus",
  ...
  brandStory: {
    ...
    variants: {
      ...
      "quiet-manifesto": variant("quiet-manifesto", "story", "정숙한 선언문", ["cta/statement-text", "story/dark-statement"], "이미지 없이 넉넉한 상하 패딩과 좁은 본문 폭(최대 560px)의 문장 블록. 문단 사이 여백을 크게 두어 읽는 속도를 늦춘다."),
  ...
  imageText: {
    ...
    variants: {
      ...
      "wide-caption": variant("wide-caption", "story", "와이드 컷+캡션", ["story/lookbook-row"], "가로로 넓은 이미지 한 장(비율 16/9~21/9) 아래 짧은 캡션과 링크. 말을 아끼고 장면으로 설명한다."),
    },
  },
  editorialBanner: {
    id: "editorialBanner",
  ...
  materials: {
    ...
    variants: {
      ...
      "text-columns": variant("text-columns", "trust", "소재 텍스트 컬럼", ["product-context/curation-band"], "사진 없이 2~3 컬럼의 짧은 텍스트 블록으로 소재와 그 선택 이유를 적는다. 차분한 타이포 위주."),
    },
  },
  process: {
    id: "process",
  ...
  infoGuide: {
    ...
    axes: { alignment: true, mediaPosition: false, density: true, tone: true },
    variants: {
      "info-columns": variant("info-columns", "trust", "인포 컬럼", ["product-context/curation-band"], "제품 철학, 사용 안내, 관리법 같은 도움말 성격의 2~3 컬럼 텍스트 블록. 차분한 타이포와 아이콘 없이 텍스트 위주."),
