...
- The slot wrapper must be a full-width block child of its section. Everything injected into it is sized as a percentage of that wrapper, so a narrow frame shrinks the thumbnails and the product type with it. Do not make the slot's parent a multi-column grid or a row flexbox that would leave the slot in one track, and do not put any absolute width or max-width below 720px on the slot or on any element that contains it. A heading block beside the products belongs in its own row above them, not in a column next to them.
