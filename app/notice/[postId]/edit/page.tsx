...
export const metadata: Metadata = { title: "공지사항 수정 — MOLIVE" };

export default async function Page({ params }: { params: Promise<{ postId: string }> }) {
  const { postId } = await params;
  await requireResourceAdminPage(`/notice/${postId}/edit`);
  const post = await findResourcePost("notice", postId);
  if (!post) notFound();
  return <ResourcePostEditor board="notice" post={post} />;
}
