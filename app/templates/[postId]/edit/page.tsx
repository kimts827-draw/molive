...
export const metadata: Metadata = { title: "템플릿 수정 — MOLIVE" };

export default async function Page({ params }: { params: Promise<{ postId: string }> }) {
  const { postId } = await params;
  await requireResourceAdminPage(`/templates/${postId}/edit`);
  const post = await findResourcePost("template", postId);
  if (!post) notFound();
  return <ResourcePostEditor board="template" post={post} />;
}
