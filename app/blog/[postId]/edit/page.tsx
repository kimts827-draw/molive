...
export const metadata: Metadata = { title: "블로그 수정 — MOLIVE" };

export default async function Page({ params }: { params: Promise<{ postId: string }> }) {
  const { postId } = await params;
  await requireResourceAdminPage(`/blog/${postId}/edit`);
  const post = await findResourcePost("blog", postId);
  if (!post) notFound();
  return <ResourcePostEditor board="blog" post={post} />;
}
